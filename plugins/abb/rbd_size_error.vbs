...
Sub ReadStore(oFile)
  ...
  For Each oChn in oaChnHDRs
    ...
    'If ( oChn.ChnLength <= oChn.Itb12 ) Then  !!!!!!!!! This line is wrong !!!!!! Correct one is below 
...
Function IsABB_LV_RBDFormat(oFile,sgFilename,sgPluginname)
  ...
  'If filesize is smaller than 128 bytes abort loading file and write an error in log file
  If ( oFile.Size < 128 ) Then RaiseError(sgFilename & " is not a "&sgPluginname&" file !")
  
  'Read the header of file
  Set oFHDR = new RBD_FILE_HEADER
  If (oFHDR.ReadFromFile(oFile) <> True) Then RaiseError (oFHDR.ReadFromFile(oFile))
...
Class RBD_FILE_HEADER
  ...
  Public Function ReadFromFile(oFile)
    ...
    Else
      'Modification v1.5
      ReadFromFile = "Timestring has no valid format"
    End if
