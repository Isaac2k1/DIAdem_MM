...
'-------------------------------------------------------------------------------
'summary: saving a .pdf mechanical report to a specific location
...
Sub CreateMechReport(templatePath, outFile, outName, strGroup, ChannelNames,NewChannelNames,MaxNumbOfChn,DiagramIndex)
  ...
  'Call PicPDFExport(outputFolder & "\PDF\" & outFile & ".pdf",0)             'Save as PDF UNCOMMENT WHEN FIXED
...
Sub CreatePowerReport(templatePath, outPath, outputName, strGroup, ChannelNames,NewChannelNames,MaxNumbOfChn,DiagramIndex)
  ...
  'Call PicPDFExport(outputFolder & "\PDF\" & outPath & ".pdf",0)             'Save as PDF UNCOMMENT WHEN FIXED
  Call GraphDeleteAll()
end Sub
'-------------------------------------------------------------------------------
...
Sub CreateExcelFile(outFile,strGroup,strChannels(),numberOfChannels)
 ...
 ' msgbox(strBleistift)
  ...
  'Call ExcelExport(outputFolder & "\\" & outFile & ".xls", "DIAdem", 0, "")  UNCOMMENT WHEN FIXED
  ...
  for i = 1 to channelLength
    ...
    for j = 2 to maxColumn
      ...
      if isZero(j) Then
        ...
        Number = CHD(i,strGroup&"/"&strChannels(j))
