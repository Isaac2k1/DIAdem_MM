...
'
  ...
  'F.Maurizio 31.03.2010/24.08.2010
  'add group properties if they do not exist
  
  'F.Maurizio 07.07.2010
  ' "load single channels" function added and msgboxes if a shot is not found
  
  'A.Huang 13.09.2012
  'New load functionality: fixed incremental load i.e: 1234/15:5:50 for 15, 20, 25 ... 50 
  'and 1234/15:5+:50 for 15, 16, 20, 21, 25, 26... 50, 51
  
  'J.Schmid 07.11.2014
  'When more than 1 Property File was found, Copy the properties to the DataGroup and delete it.
...
'-------------- Start of User Input Check (SerialNo) -----------------------------------------------
    ...
    'Check the syntax of SNo/TNo string
      ...
      Do While A <= Len(sCurrVal)
        ...
        If (sChar <> "1" ) And (sChar <> "2" ) And (sChar <> "3" ) And (sChar <> "4" ) And (sChar <> "5" ) And (sChar <> "6" ) And (sChar <> "+" ) And _
           (sChar <> "7" ) And (sChar <> "8" ) And (sChar <> "9" ) And (sChar <> "0" ) And (sChar <> "," ) And (sChar <> "-" ) And (sChar <> "/" ) And (sChar <> ":" )Then
...
'-------------- Start of Analysis of TestRange given by User ---------------------------------------        
      ...
      'Change Alex 13-09-2013
      If Instr(sBuffer, ":") <> 0 then
        dim begin
        dim plus
        begin = left(sBuffer, instr(sBuffer, ":")-1)
        sBuffer = mid(sBuffer, instr(sBuffer, ":")+1)
        
        ' check for only 2 :
        If Instr(sBuffer, ":") = 0 then
          msgbox "enter incremental test like this: 1234/100:5:200 for 100, 105, 110... 200 or 1234/100:5+:200 for 100, 101, 105, 106, ... 200"
          'do not allow load
          AllowLoad = 0
        else    
          dim incre
          dim ending
          dim plussign
          incre = left(sBuffer, instr(sBuffer, ":")-1)
          ending = mid(sBuffer, instr(sBuffer, ":")+1)
          plussign = 0
          if instr(incre, "+" ) <> 0 then
            plussign = 1
            incre = left(incre, instr(incre, "+")-1)
          end if
          
          if (isnumeric(begin) = 0 or isnumeric(incre) = 0 or isnumeric(ending) = 0) then
            msgbox "enter incremental test like this: 1234/100:5:200 for 100, 105, 110... 200 or 1234/100:5+:200 for 100, 101, 105, 106, ... 200"
            AllowLoad = 0
          end if
          if (len(begin)>4 or len(incre)>4 or len(ending)>4) then
            msgbox "test numbers must be at most 4 digits"
            AllowLoad = 0
          end if
          
          K = -1
          while ( Cint (begin) <=  Cint(ending))
            Redim Preserve asBuffer (K)
            L = begin
            K = K+1
            ...
            begin = Cint(begin) + Cint(incre)
          Wend
        end if
      else       
        'Change Alex ends here
        'Find out if there is a range seperated with ","
...
'-------------- End of Analysis of TestRange given by User ---------------------------------------
      ...
      If(AllowLoad) Then
          ...
          Dim MyDataFinder, AdvancedQuery, w, ww, q, sDataName2
          ...
          For J= 0 To K       
            If IsEscKeyPressed Then Exit For       
            ...
            If(MyDataFinder.Results.Count = 0) Then 'Keine Suchergebnisse
             
             if MyDataFinder.Name <> "acqui@CH-W-PTHX001" and MyDataFinder.Name <> "bacqui@CH-W-0014698" then msgbox("Choose the right Datafinder in the navigator! (e.g.: acqui@CH-W-PTHX001 or bacqui@CH-W-0014698)") 
             
             For q=1 to ShotNo(J)-1 'Schleife um herauzufinden welche die grösste Schussnummer mit dieser Serienummer ist
              sDataName2   = prefix&SerialNo&"-"&ShotNo(J)-q&"*.rbd"
              Call AdvancedQuery.Conditions.RemoveAll()          
              'Search for Binary Data File "rbd"
              Call AdvancedQuery.Conditions.Add(eSearchFile,"fileName","=",sDataName2)
              Call MyDataFinder.Search(AdvancedQuery)
            
              If(MyDataFinder.Results.Count <> 0) Then
                msgbox "No test data found for "&sDataName(J)
                msgbox("The highest shot number with this serial number is: "&SerialNo&"-"&ShotNo(J)-q&"!")
                Exit Sub
              End if
              If q = ShotNo(J)-1 and MyDataFinder.Results.Count = 0 Then 'In diesem Fall gibt es keine Schüsse mit dieser Serienummer
                msgbox "No test data found for "&sDataName(J)
                msgbox("There is any serial number: "&SerialNo)
              End if
             Next
             Exit Sub
            ...
            Elseif MyDataFinder.Results.Count = 1 Then 'Load a single rba File
              ...
              for rba_load_count=1 to load_count
                ...
                'add group properties if they do not exist  F.Maurizio, 24.08.10
                if not grouppropexist(groupcount-rba_load_count,"KIND_OF_TEST") then
                   call grouppropcreate(groupcount-rba_load_count,"KIND_OF_TEST",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"KIND_OF_TEST","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_CIRCUIT_ID") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_CIRCUIT_ID",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_CIRCUIT_ID","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_ENGINEER") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_ENGINEER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_ENGINEER","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_AREA") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_AREA",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_AREA","*****")
                end if
                ...
                if not grouppropexist(groupcount-rba_load_count,"REMARKS") then
                   call grouppropcreate(groupcount-rba_load_count,"REMARKS",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"REMARKS","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"PROJECTNAME") and not grouppropexist(groupcount-rba_load_count,"PROJECT_NAME") then
                   call grouppropcreate(groupcount-rba_load_count,"PROJECT_NAME",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"PROJECT_NAME","*****")
                end if
                if grouppropexist(groupcount-rba_load_count,"PROJECTNAME") and not grouppropexist(groupcount-rba_load_count,"PROJECT_NAME") then
                   call grouppropcreate(groupcount-rba_load_count,"PROJECT_NAME",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"PROJECT_NAME",GroupPropValGet(groupcount-rba_load_count,"PROJECTNAME"))
                end if
                if not grouppropexist(groupcount-rba_load_count,"TESTOBJECT") and not grouppropexist(groupcount-rba_load_count,"TEST_OBJECT") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_OBJECT",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_OBJECT","*****")
                end if
                if grouppropexist(groupcount-rba_load_count,"TESTOBJECT") and not grouppropexist(groupcount-rba_load_count,"TEST_OBJECT") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_OBJECT",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_OBJECT",GroupPropValGet(groupcount-rba_load_count,"TESTOBJECT"))
                end if
                if not grouppropexist(groupcount-rba_load_count,"TESTED_CHAMBERS") then
                   call grouppropcreate(groupcount-rba_load_count,"TESTED_CHAMBERS",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TESTED_CHAMBERS","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"NO__OF_CHAMBERS") then
                   call grouppropcreate(groupcount-rba_load_count,"NO__OF_CHAMBERS",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"NO__OF_CHAMBERS","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"CLIENT") then
                   call grouppropcreate(groupcount-rba_load_count,"CLIENT",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"CLIENT","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_ADVISOR") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_ADVISOR",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_ADVISOR","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"MANUFACTURER") then
                   call grouppropcreate(groupcount-rba_load_count,"MANUFACTURER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"MANUFACTURER","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_TYPE") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_TYPE",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_TYPE","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_NUMBER") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_NUMBER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_NUMBER","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TO_VERSION") then
                   call grouppropcreate(groupcount-rba_load_count,"TO_VERSION",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TO_VERSION","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"DRAWING_NUMBER") then
                   call grouppropcreate(groupcount-rba_load_count,"DRAWING_NUMBER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"DRAWING_NUMBER","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"ACCOUNT") then
                   call grouppropcreate(groupcount-rba_load_count,"ACCOUNT",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"ACCOUNT","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"STANDARD") then
                   call grouppropcreate(groupcount-rba_load_count,"STANDARD",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"STANDARD","*****")
                end if
                ...
                if not grouppropexist(groupcount-rba_load_count,"SERIAL_NO__TO") then
                   call grouppropcreate(groupcount-rba_load_count,"SERIAL_NO__TO",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"SERIAL_NO__TO","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"U_RATED__KV__") then
                   call grouppropcreate(groupcount-rba_load_count,"U_RATED__KV__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"U_RATED__KV__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"I_SC_RATED__KA__") then
                   call grouppropcreate(groupcount-rba_load_count,"I_SC_RATED__KA__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"I_SC_RATED__KA__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"F_RATED__HZ__") then
                   call grouppropcreate(groupcount-rba_load_count,"F_RATED__HZ__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"F_RATED__HZ__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TEST_CURRENT__KA__") then
                   call grouppropcreate(groupcount-rba_load_count,"TEST_CURRENT__KA__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TEST_CURRENT__KA__","*****")
                end if
                ...
                if not grouppropexist(groupcount-rba_load_count,"FILLING_PRESSURE__BAR__") then
                   call grouppropcreate(groupcount-rba_load_count,"FILLING_PRESSURE__BAR__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"FILLING_PRESSURE__BAR__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TYPE_OF_DRIVE") then
                   call grouppropcreate(groupcount-rba_load_count,"TYPE_OF_DRIVE",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TYPE_OF_DRIVE","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"SERIAL_NO__DRIVE") then
                   call grouppropcreate(groupcount-rba_load_count,"SERIAL_NO__DRIVE",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"SERIAL_NO__DRIVE","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"U_MOTOR__V__") then
                   call grouppropcreate(groupcount-rba_load_count,"U_MOTOR__V__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"U_MOTOR__V__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"U_MOTOR_TYPE") then
                   call grouppropcreate(groupcount-rba_load_count,"U_MOTOR_TYPE",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"U_MOTOR_TYPE","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TRIPPING_VOLTAGE__V__") then
                   call grouppropcreate(groupcount-rba_load_count,"TRIPPING_VOLTAGE__V__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TRIPPING_VOLTAGE__V__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"STROKE_DOWN__MM__") then
                   call grouppropcreate(groupcount-rba_load_count,"STROKE_DOWN__MM__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"STROKE_DOWN__MM__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"STROKE_UP__MM__") then
                   call grouppropcreate(groupcount-rba_load_count,"STROKE_UP__MM__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"STROKE_UP__MM__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"TOTAL_STROKE__MM__") then
                   call grouppropcreate(groupcount-rba_load_count,"TOTAL_STROKE__MM__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"TOTAL_STROKE__MM__","*****")
                end if
                if not grouppropexist(groupcount-rba_load_count,"DRIVE_STROKE__MM__") then
                   call grouppropcreate(groupcount-rba_load_count,"DRIVE_STROKE__MM__",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"DRIVE_STROKE__MM__","*****")
                end if  
                if not grouppropexist(groupcount-rba_load_count,"MEASUR__PLACE") then
                   call grouppropcreate(groupcount-rba_load_count,"MEASUR__PLACE",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"MEASUR__PLACE","*****")
                end if  
                if not grouppropexist(groupcount-rba_load_count,"_ST_OBSERVER") then
                   call grouppropcreate(groupcount-rba_load_count,"_ST_OBSERVER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"_ST_OBSERVER","*****")
                end if  
                if not grouppropexist(groupcount-rba_load_count,"_ND_OBSERVER") then
                   call grouppropcreate(groupcount-rba_load_count,"_ND_OBSERVER",Datatypestring)
                   call grouppropset(groupcount-rba_load_count,"_ND_OBSERVER","*****")
                end if  
            ...
            Elseif MyDataFinder.Results.Count > 1 Then 'Load multiple rba Files
              ...
              Navigator.LoadData(MyDataFinder.Results)
					dim xi								'Copies the Properties to the DataGroup and deletes the Property Group
					For xi = 1 to NoOfRBDLoaded
						Call GroupPropCopy(NoOfRBDLoaded+1,xi )
						call groupdel(NoOfRBDLoaded+1)
					Next
