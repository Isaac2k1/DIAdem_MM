...
'Properties of the original channel is transferred to result vaweform channel
  ...
  While(iChn<=ChNum)
   ...
   Else
    ...
    Do While(iGroupindex(iChn)=iGrp)
     ...
     If(TbACopy <> TbBCopy) and (iTb21 > iTb12) and (LastXCopy >= iTb21)  Then      
...
'!?! ElseIf(TbACopy = TbBCopy) and (iTb21 = iTb12) and (iTb12 >= LastXCopy) Then      
'!?! The condition (iTb12 >= LastXCopy) is not met in 7000/74 and is very likely not needed at all. MM
     ...
     'Calculate scaled X channel values (scaled_value = Timestart + SampleNo*TimebaseA) 
     'Call CHNLINSCALE(GHdChnName,GHdChnName&"_scaled",TbACopy,tStart) '... Y,E,CHNSCALEFACTOR,CHNSCALEOFFSET 
     'Calculate scaled Y Channel values (Scaled_Y = yScale*(SampleValue-yOffset))
     'Call CHNLINSCALE(sChnNameYinterp,sChnNameYinterp&"_scaled",yScale,(-1)*(yScale*yOffset)) '... Y,E,CHNSCALEFACTOR,CHNSCALEOFFSET
