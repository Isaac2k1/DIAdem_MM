...
'v1.9.4 (Adrian Kress)
' Offset-Error deleted
'
'v1.9.5 (Adrian Kress)
' L1,L2 and L3 adjust calculation with those variables
'
'-----------------------------------------------------------------------------------------------------------------------
'Dim xy(0,1)
'xy(0,0) = 3
...
Function equix(iGroupindex(),iXYChNo(),iGenType,CustomTimeStep)
    ...
    call uiautorefreshset(false) 'disable refresh of data portal, about 15% speed gain
    iChn = 0
          ...
          iGrp = iGroupindex(iChn)
            ...
            Do While(iGroupindex(iChn)=iGrp)
                ...
                If(TbACopy <> TbBCopy) and (iTb21 = iTb12)  then
                  ...
                  if (SmallSR*(10^(9)) < 2147483647) and (t_total*(10^(9)) < 2147483647) then
                    L1 = Fix(SmallSR*(10^(9))) 
                    L2 = Fix(t_total*(10^(9)))
                  else
                    L1 = Fix(SmallSR*(10^(5))) 
            ...
            'offset adjustment ------ cause of occuring errors, this part is comment out 29.03.2017 -----
...
' This code is no longer necessary. It was very likely anyway toally wrong
' see history note to v1.9.4
'  
  ...
  'Function mean(channel,startXValue,stopXValue) 
      ...
      'dim i
      'int stepWidth 
      ''msgbox("start:"&startXValue&", stop:"&stopXValue&", width:"&stepWidth)
